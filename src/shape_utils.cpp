...
std::optional<Shape> MakeCircle(const std::vector<double> &v) {
    return std::optional<bool>{true}
        .and_then([&](bool) -> std::optional<bool> {
            if (v.size() != 3)
                return std::nullopt;
            return true;
        })
        .and_then([&](bool) -> std::optional<Shape> {
            if (v[2] <= 0)
                return std::nullopt;
            return Circle{{v[0], v[1]}, v[2]};
        });
...
std::optional<Shape> MakeLine(const std::vector<double> &v) {
    return std::optional<bool>{true}.and_then([&](bool) -> std::optional<Shape> {
        if (v.size() != 4)
            return std::nullopt;
        return Line{{v[0], v[1]}, {v[2], v[3]}};
    });
...
std::optional<Shape> MakePolygon(const std::vector<double> &v) {
    return std::optional<bool>{true}
        ...
        .and_then([&](bool) -> std::optional<int> {
            auto sides_opt = ParseDouble(std::to_string(v[3]));
            if (!sides_opt.has_value())
                return std::nullopt;
            return static_cast<int>(v[3]);
        })
        .and_then([&](int sides) -> std::optional<Shape> {
            if (sides < 3)
                return std::nullopt;
            return RegularPolygon{{v[0], v[1]}, v[2], sides};
        });
...
std::optional<size_t> FindHighestShape(std::span<const Shape> shapes) {
    auto res_it = std::ranges::max_element(
        shapes, [](const auto &lhs, const auto &rhs) { return queries::GetHeight(lhs) < queries::GetHeight(rhs); });

    if (res_it != shapes.end())
        return std::distance(shapes.begin(), res_it);
    return std::nullopt;
}
}  // namespace geometry::utils